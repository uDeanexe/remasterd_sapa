...
Artisan::command('users:prune-non-admin {--force : Lewati konfirmasi}', function () {
    $adminCount = (int) User::query()->where('role', 'administrator')->count();
    if ($adminCount === 0) {
        $this->error('Tidak ada user role=administrator. Batalkan untuk mencegah lockout.');
        return 1;
    }

    $nonAdminCount = (int) User::query()->where('role', '!=', 'administrator')->count();
    if ($nonAdminCount === 0) {
        $this->info('Tidak ada user non-admin untuk dihapus.');
        return 0;
    }

    if (! $this->option('force')) {
        if (! $this->confirm("Hapus {$nonAdminCount} user non-admin? (Admin tetap tersisa: {$adminCount})", false)) {
            $this->warn('Dibatalkan.');
            return 0;
        }
    }

    $deleted = (int) User::query()->where('role', '!=', 'administrator')->delete();
    $this->info("Selesai. User non-admin terhapus: {$deleted}");

    return 0;
})->purpose('Hapus semua user selain administrator');
