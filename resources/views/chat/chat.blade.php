<x-app-layout>
    ...
    @php
        $activeTab = (string) request()->query('tab', 'group');
        $activeTab = in_array($activeTab, ['group', 'personal'], true) ? $activeTab : 'group';
        $selectedUserId = (int) request()->query('user_id', 0);
        $supportsPrivate = \Illuminate\Support\Facades\Schema::hasTable('chat_recipients');
    ...
    <div class="h-full overflow-hidden flex flex-col bg-gradient-to-br from-gray-50 via-white to-blue-50 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900">
        <div class="flex-1 min-h-0 flex flex-col overflow-hidden p-2 sm:p-4 lg:p-6">
            <div class="chat-panel w-full bg-white shadow-xl shadow-slate-200/50 h-full min-h-0 flex flex-col border border-slate-100 rounded-3xl overflow-hidden relative dark:bg-slate-900/70 dark:border-slate-800 dark:shadow-black/40">
                ...
                <header class="border-b border-slate-100 bg-white/95 backdrop-blur-md sticky top-0 z-20 dark:border-slate-800 dark:bg-slate-900/85">
                    <div class="flex w-full items-center justify-between px-6 py-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h1 class="font-bold text-lg text-slate-800 tracking-tight dark:text-slate-100">{{ $activeTab === 'personal' ? 'Chat Personal' : 'Chat Grup' }}</h1>
                                <p class="text-xs text-slate-500 font-medium dark:text-slate-300">{{ $activeTab === 'personal' ? 'Pilih lawan chat, lalu kirim pesan private.' : 'Ruang obrolan umum untuk semua pengguna.' }}</p>
                            </div>
                        </div>
                        <div class="hidden sm:flex flex-nowrap items-center gap-3">
                            <div class="inline-flex items-center rounded-full border border-slate-200 bg-slate-50 p-1 text-xs font-bold text-slate-600 dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-200">
                                <a href="{{ request()->fullUrlWithQuery(['tab' => 'group', 'user_id' => null]) }}"
                                   class="rounded-full px-3 py-1.5 transition {{ $activeTab === 'group' ? 'bg-white text-indigo-700 shadow-sm dark:bg-slate-900' : 'hover:bg-white/70 dark:hover:bg-white/5' }}">
                                    Grup
                                </a>
                                <a href="{{ request()->fullUrlWithQuery(['tab' => 'personal']) }}"
                                   class="rounded-full px-3 py-1.5 transition {{ $activeTab === 'personal' ? 'bg-white text-indigo-700 shadow-sm dark:bg-slate-900' : 'hover:bg-white/70 dark:hover:bg-white/5' }} {{ $supportsPrivate ? '' : 'pointer-events-none opacity-50' }}"
                                   title="{{ $supportsPrivate ? 'Chat private' : 'Private chat belum aktif (migrate chat_recipients)' }}">
                                    Personal
                                </a>
                            </div>
                            ...
                            <div id="personal-target-wrap" class="{{ $activeTab === 'personal' ? '' : 'hidden' }}">
                                <select id="personal-target" class="rounded-full border-slate-200 bg-slate-50 py-2 pl-3 pr-9 text-xs font-semibold text-slate-700 focus:bg-white focus:border-indigo-400 focus:ring-indigo-200 dark:border-slate-700 dark:bg-slate-950/60 dark:text-slate-100">
                    ...
                    <div class="sm:hidden px-6 pb-3">
                        <div class="inline-flex w-full items-center justify-center rounded-full border border-slate-200 bg-slate-50 p-1 text-xs font-bold text-slate-600 dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-200">
                            <a href="{{ request()->fullUrlWithQuery(['tab' => 'group', 'user_id' => null]) }}"
                               class="flex-1 text-center rounded-full px-3 py-2 transition {{ $activeTab === 'group' ? 'bg-white text-indigo-700 shadow-sm dark:bg-slate-900' : 'hover:bg-white/70 dark:hover:bg-white/5' }}">
                                Grup
                            </a>
                            <a href="{{ request()->fullUrlWithQuery(['tab' => 'personal']) }}"
                               class="flex-1 text-center rounded-full px-3 py-2 transition {{ $activeTab === 'personal' ? 'bg-white text-indigo-700 shadow-sm dark:bg-slate-900' : 'hover:bg-white/70 dark:hover:bg-white/5' }} {{ $supportsPrivate ? '' : 'pointer-events-none opacity-50' }}">
                                Personal
                            </a>
                        </div>
                    </div>
                ...
                @if($latestPinned && $chatContext === 'admin' && $activeTab === 'group')
                    <!-- Pinned Messages Bar -->
                ...
                <div id="chat-container" class="chat-message-list flex-1 overflow-y-auto bg-slate-50/50 dark:bg-slate-950/30" role="log">
                    ...
                    @foreach($messages as $msg)
                        ...
                        @php
                            $recipientIds = $msg->relationLoaded('recipients')
                                ? $msg->recipients->pluck('id')->map(fn ($id) => (int) $id)->values()->all()
                                : [];
                            $isPrivate = ! empty($recipientIds);
                        @endphp

                        <div
                            id="msg-{{ $msg->id }}"
                            class="chat-message-row group flex flex-col {{ $isMe ? 'items-end' : 'items-start' }}"
                            data-chat-row
                            data-chat-id="{{ (int) $msg->id }}"
                            data-chat-private="{{ $isPrivate ? '1' : '0' }}"
                            data-chat-sender="{{ (int) $msg->user_id }}"
                            data-chat-recipients='@json($recipientIds)'
                ...
                <footer class="sticky bottom-0 z-30 bg-white/95 backdrop-blur-md border-t border-slate-100 pb-[calc(env(safe-area-inset-bottom)+1rem)] dark:bg-slate-900/85 dark:border-slate-800">
                    <form id="chat-form" action="{{ route('web.chats.store') }}" method="POST" enctype="multipart/form-data" class="w-full space-y-3 relative px-3 sm:px-6 py-4">
                        ...
                        <div id="personal-target-wrap-mobile" class="{{ $activeTab === 'personal' ? '' : 'hidden' }} sm:hidden">
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1">Chat Personal</label>
                            <select id="personal-target-mobile" class="form-control">
                                <option value="0">Pilih user…</option>
                                @foreach($mentionUsers as $u)
                                    <option value="{{ $u['id'] }}" @selected($selectedUserId === (int) $u['id'])>{{ $u['name'] }}</option>
                                @endforeach
                            ...
                            @if(! $supportsPrivate)
                                <p class="mt-2 text-xs text-rose-600">Private chat belum aktif. Jalankan migrate untuk membuat tabel `chat_recipients`.</p>
                            @endif
                        </div>
    ...
    const supportsPrivate = @json($supportsPrivate);
    const selectedUserIdFromQuery = @json((int) $selectedUserId);
    ...
    onDomReady(() => {
        ...
        const personalTargetDesktop = document.getElementById('personal-target');
        const personalTargetMobile = document.getElementById('personal-target-mobile');

        function getPersonalTargetEl() {
            return personalTargetMobile || personalTargetDesktop;
        }

        function getPersonalTargetId() {
            const el = getPersonalTargetEl();
            if (!el) return 0;
            const id = parseInt(String(el.value || '0'), 10);
        ...
        function applyChatTabFilter() {
            ...
            const targetId = getPersonalTargetId();

            rows.forEach((row) => {
                const isPrivate = row.getAttribute('data-chat-private') === '1';
                const senderId = parseInt(row.getAttribute('data-chat-sender') || '0', 10) || 0;
                let recipients = [];
                try { recipients = JSON.parse(row.getAttribute('data-chat-recipients') || '[]') || []; } catch (_) { recipients = []; }
                ...
                const isFromMeToTarget = senderId === currentUserId && recipients.includes(targetId);
                const isFromTargetToMe = senderId === targetId && recipients.includes(currentUserId);
                row.classList.toggle('hidden', !(isFromMeToTarget || isFromTargetToMe));
            });
        }

        if (activeTab === 'personal') {
            const el = getPersonalTargetEl();
            if (el && selectedUserIdFromQuery && parseInt(String(el.value || '0'), 10) !== selectedUserIdFromQuery) {
                el.value = String(selectedUserIdFromQuery);
            }
        }

        const personalTargetEl = getPersonalTargetEl();
        if (personalTargetEl) {
            personalTargetEl.addEventListener('change', () => {
                const id = getPersonalTargetId();
                setQueryParam('user_id', id || null);
            });
        }

        applyChatTabFilter();
        try {
            const obs = new MutationObserver(() => applyChatTabFilter());
    ...
    function scrollToMessage(id) {
        ...
        // Personal chat: ensure the message contains a private mention token so backend sets chat_recipients.
        if (activeTab === 'personal') {
            if (!supportsPrivate) {
                event.preventDefault();
                showChatToast('Chat Personal', 'Private chat belum aktif. Jalankan migrate untuk membuat chat_recipients.');
                return;
            }

            const targetEl = document.getElementById('personal-target-mobile') || document.getElementById('personal-target');
            const targetId = targetEl ? parseInt(String(targetEl.value || '0'), 10) : 0;
            const hasText = String(messageInput.value || '').trim() !== '';
            const hasFile = ((fileInput.files && fileInput.files.length > 0) || !!pendingFile);

            if ((!targetEl || !Number.isFinite(targetId) || targetId <= 0) && (hasText || hasFile)) {
                event.preventDefault();
                showChatToast('Chat Personal', 'Pilih user terlebih dahulu.');
                try { targetEl?.focus(); } catch (_) {}
                return;
            }

            if (targetEl && Number.isFinite(targetId) && targetId > 0) {
                const targetName = (targetEl.options && targetEl.selectedIndex >= 0)
                    ? String(targetEl.options[targetEl.selectedIndex]?.text || '')
                    : '';
                const token = `@[${targetId}|${targetName}]`;
                const current = String(messageInput.value || '');
                if (!current.includes(`@[${targetId}|`)) {
                    messageInput.value = `${token} ${current.trimStart()}`.trim();
                }
            }
        }
