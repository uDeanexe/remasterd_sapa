            ...
            <div class="admin-page-header">
                ...
                <div class="admin-page-header-body">
                    <div>
                        ...
                        <p class="admin-subtitle">Tambah karyawan, atur role, dan pantau status password akun.</p>
                    </div>
                            ...
                            @forelse($users as $user)
                                <tr class="data-table-row">
                                    <td class="min-w-56">
                                        ...
                                        @endphp
                                        <p class="mt-1 text-xs text-slate-500">{{ $roleLabels[$user->role] ?? ucfirst((string) $user->role) }}</p>
                                    </td>
                    ...
                    <div class="modal-header">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p class="modal-subtitle">Password default otomatis diset ke {{ $defaultPassword ?? 'jonusa123' }}.</p>
                            </div>
                    ...
                    <form method="POST" action="{{ route('users-management.store') }}" class="modal-body space-y-4" data-submit-lock>
                        ...
                        <label>
                            ...
                            @php
                                ...
                                if (isset($roles) && $roles && $roles->count()) {
                                    foreach ($roles as $roleRow) {
                                        $roleId = (string) $roleRow->role_id;
                                        $roleOptions[$roleId] = (string) ($roleRow->role_name ?: $roleId);
                                    }
                                }
                            @endphp
                            @if($roleOptions)
                                <select name="role" class="form-control" required>
                                    @foreach($roleOptions as $value => $label)
                                        <option value="{{ $value }}" @selected(old('role', 'teknisi') === $value)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            @else
                                <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                                    Anda tidak punya hak untuk membuat user baru.
                                </div>
                            @endif
                            <p class="mt-1 text-xs text-slate-500">Role menentukan menu & hak akses. Divisi otomatis mengikuti role (fixed).</p>
                        </label>
